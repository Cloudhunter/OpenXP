...
public class LanguageUtils {
	public static void setupLanguages() {
		...
		try {
			InputStream input = CommonProxy.class.getResourceAsStream(
					String.format("%s/languages.txt", OpenXP.LANGUAGE_PATH)
			);
		...
		while (iterator.hasNext()) {
			...
			URL url = CommonProxy.class.getResource(
					String.format("%s/%s.lang", OpenXP.LANGUAGE_PATH, langString)
			);
			if (url == null) {
				continue;
			}
